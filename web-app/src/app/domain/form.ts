 ...
 * Copie côté mobile : app/src/features/coach/form.ts (teintes de puces à part) — garder les deux alignés.
...
export const FORM_STATE_STYLE: Record<FormState, { label: string; chip: string; order: number }> = {
  overreached: { label: 'Surmené', chip: 'chip-danger', order: 0 },
  loaded: { label: 'Chargé', chip: 'chip-warn', order: 1 },
  inactive: { label: 'Inactif', chip: '', order: 2 },
  rested: { label: 'Reposé', chip: 'chip-accent', order: 3 },
  optimal: { label: 'Dans la zone', chip: 'chip-done', order: 4 },
  unknown: { label: 'À découvrir', chip: '', order: 5 },
};
