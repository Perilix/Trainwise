...
import { addDays, daysBetween, isoWeek, startOfWeek } from '../core/dates';
import { formatDayMonthYear, formatDayShort, formatMonthName, formatMonthShort, formatMonthYear, formatTime, paceToSeconds, parseDay, toIsoDay } from '../core/format';

import { comparePhases } from './planned-vs-done';
...
export function mapRunDetail(run: ApiRun, coachName?: string, vma?: number): RunDetail {
  ...
  // La séance prévue, figée dans la sortie au moment du rapprochement : c'est
  // elle qui permet la lecture « prévu / réalisé » phase par phase.
  const plannedBlocks = run.plannedSnapshot?.runBlocks ?? [];
  const planned = plannedBlocks.length
    ? {
        title: run.plannedSnapshot?.title ?? undefined,
        blocks: describeBlocks(plannedBlocks, vma),
        segments: blocksToSegments(plannedBlocks, vma),
        phases: comparePhases(plannedBlocks, runBlocks),
      }
    : undefined;
...
/** Les 7 jours d'une semaine, à partir d'un ou deux mois déjà construits. */
/** La compétition à venir la plus proche, avec le compte à rebours. */
export function nextCompetition(competitions: ApiCompetition[], now: Date) {
  const today = toIsoDay(now);
  const next = competitions
    .map((competition) => ({ ...competition, day: dayOf(competition.date) }))
    .filter((competition) => competition.status !== 'cancelled' && competition.day >= today)
    .sort((a, b) => a.day.localeCompare(b.day))[0];
  if (!next) return null;
  const days = daysBetween(now, parseDay(next.day));
  return {
    name: next.name,
    date: next.day,
    priority: next.priority,
    days,
    countdown: days === 0 ? "Aujourd'hui" : `J-${days}`,
  };
}

export function buildWeekPlan(months: PlanningMonth[], start: Date, now: Date): WeekPlanDay[] {
  const sessionsByDay: Record<string, PlannedSession[]> = {};
  const activitiesByDay: Record<string, Activity[]> = {};
  for (const month of months) {
    Object.assign(sessionsByDay, month.sessionsByDay);
    Object.assign(activitiesByDay, month.activitiesByDay);
  }
  const today = toIsoDay(now);
  return Array.from({ length: 7 }, (_, index) => {
    const date = new Date(start.getFullYear(), start.getMonth(), start.getDate() + index);
    const iso = toIsoDay(date);
    return { iso, isToday: iso === today, sessions: sessionsByDay[iso] ?? [], activities: activitiesByDay[iso] ?? [] };
  });
}
...
export function mapMessages(messages: ApiMessage[], myId: string, now: Date): ChatMessage[] {
  ...
  return messages.map((message) => {
    ...
    return {
      ...
      text:
        message.type === 'text' || message.type === 'session' || message.type === 'system'
          ? message.content
          : message.type === 'image'
            ? 'Photo'
