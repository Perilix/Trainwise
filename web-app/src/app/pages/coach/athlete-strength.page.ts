...
/** Séance de renforcement réalisée par un athlète, vue par le coach. */
@Component({
      ...
      @if (session.loading()) {
        ...
        <div class="cols">
          ...
          <div class="col">
            ...
            @if (data.notes) {
              <section class="card card-pad">
                <span class="h2">Notes de l'athlète</span>
...
export class CoachAthleteStrengthPage {
  ...
  readonly session = load(() => this.coach.athleteStrengthSession$(this.id(), this.sessionId()));
  ...
  readonly title = computed(() => {
    ...
    return type ? (labels[type] ?? 'Renforcement') : 'Renforcement';
  });

  readonly totalSets = computed(() =>
    (this.session.data()?.exercises ?? []).reduce((total, entry) => total + entry.sets.length, 0),
  );
